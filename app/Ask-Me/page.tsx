...
function ChatUI({ userId, userName }: { userId: string; userName: string }) {
  ...
  return (
    <div className="flex flex-col px-8 pb-8 max-w-3xl" style={{ height: "calc(100dvh - 5rem)" }}>
      ...
      {limitReached && (
        <div className="shrink-0 mb-3 px-4 py-3 rounded-xl bg-violet-500/10 border border-violet-500/20 text-violet-600 dark:text-violet-400 text-sm">
          <span>
            You&apos;ve reached the {usage?.limit}-message limit for today. Still have questions?{" "}
            <a href="/Contact"
              className="inline-flex items-center px-2.5 py-1 rounded-lg bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-xs font-medium hover:opacity-75 transition-opacity">
              Contact me
            </a>
          </span>
        </div>
